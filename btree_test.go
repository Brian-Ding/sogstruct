...
func TestBTree2(t *testing.T) {
	...
	if want, got := 50, tree.root.keys[1]; got != want {
		t.Errorf("key of btree's root is not [50]")
	}

	if want, got := 20, tree.root.children[0].keys[0]; got != want {
		t.Errorf("key of btree's children[0] is not [20]")
	}
	...
	if want, got := 60, tree.root.children[2].keys[0]; got != want {
		t.Errorf("key of btree's children[2] is not [60]")
	}

	if want, got := 70, tree.root.children[2].keys[1]; got != want {
		t.Errorf("key of btree's children[2] is not [70]")
	}
}
